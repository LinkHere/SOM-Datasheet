...
for itrs, row in enumerate(rows, 1):
    ...
    if row.Vaccine_Id is not None:
        ...
        btn_state = "disabled"
    st.markdown(f"""<span class="badge text-bg-danger">itrs</span>""", unsafe_allow_html=True) student = st.checkbox(label=f'{row.Last_Name}, {row.First_Name} {row.Middle_Initial} ', key=itrs)
    if student:
        st.markdown(f"""
